	...
	try {
		...
		if (order) {
			...
			if (newStatus && Object.keys(newStatus).length > 0) {
				const statusArr = order.status;
				order.status = [...statusArr, newStatus];
				const updatedOrder = await order.save();
				await db.disconnect();
				return res.json({ status: 200, order: updatedOrder });
			} else {
				return res.json({
					status: 200,
					message: 'Provide valid order status',
				});
			}
		} else {
			await db.disconnect();
...
async function updateProduct(req, res) {
	...
	try {
		...
		if (req.body.name) {
			...
			data['slug'] = req.body.name.split(' ').join('-');
		}
		if (req.body.price) {
			data['price'] = req.body.price;
		}

		if (req.body.masterCategory) {
			data['masterCategory'] = req.body.masterCategory;
		}
		if (req.body.brand) {
		...
		if (req.body.avialableColors) {
			data['avialableColors'] = req.body.avialableColors;
		}
		if (req.body.description) {
			data['description'] = req.body.description;
		}
		if (req.body.availableSizes) {
			data['availableSizes'] = req.body.availableSizes;
		}
		await db.connect();
...
router.route('/orders/status/:orderId').put(verifyUser, updateOrderStatus);
